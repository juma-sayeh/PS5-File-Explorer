...
#ifdef BFPILOT_UNRAR_STREAM
  if (bfpilot_unrar_stream_active() && !Cmd->UseStdin.empty())
  {
    if (!SplitHeader && HeaderType!=HEAD_ENDARC)
      return false;
    if (HeaderType==HEAD_ENDARC && !Arc.EndArcHead.NextVolume)
      return false;

    std::wstring NextName=Arc.FileName;
    NextVolumeName(NextName,!Arc.NewNumbering);
    Arc.FileName=NextName;

...
typedef struct archive_app_prepare_info {
  int  found;
  int  prepared;
  int  permissions_ok;
  int  path_ok;
  ...
  int  target_exists;
  int  merge_required;
  int  delete_extra_files;
  int  delete_extra_dirs;
  char title_id[64];
  char scan_path[1024];
  char app_path[1024];
  char prepared_path[1024];
  char delete_extra_sample[512];
  char reason[160];
} archive_app_prepare_info_t;
...
int archive_backup_remove_handler(const http_request_t *req);
...
int archive_place_stage(const char *stage, const char *archive_name,
                        ...
                        char *err, size_t err_size);
int archive_app_prepare_probe(const char *path,
                              archive_app_prepare_info_t *info,
                              char *err, size_t err_size);
int archive_app_prepare_apply(const char *path,
                              archive_app_prepare_info_t *info,
                              char *err, size_t err_size);
